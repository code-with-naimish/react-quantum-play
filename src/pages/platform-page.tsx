...
const PlatformPage = () => {
  ...
  const getGames = async (platformId?: string) => {
    if (!platformId) {
    ...
    try {
      const response = await fetch(`https://free-to-play-games-database.p.rapidapi.com/api/games?platform=${platformId}`, fetchOptions);
      if (!response.ok) {
